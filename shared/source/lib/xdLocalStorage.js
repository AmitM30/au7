...
window.xdLocalStorage = window.xdLocalStorage || (function () {
  ...
  function init(customOptions) {
    options = XdUtils(customOptions, options);
  ...
  return {
    ...
    readCookie: function (key, callback) {
      if (!isApiReady()) {
        return;
      }
      buildMessage('readCookie', key, null, callback);
    },
    readCart: function (key, callback) {
      if (!isApiReady()) {
        return;
      }
      buildMessage('readCart', key, null, callback);
    },
    writeCookie: function (key, data, callback) {
      if (!isApiReady()) {
        return;
      }
      buildMessage('writeCookie', key, data, callback);
    },
    writeCart: function (key, data, callback) {
      if (!isApiReady()) {
        return;
      }
      buildMessage('writeCart', key, data, callback);
    },
    clearCookie: function (key, callback) {
      if (!isApiReady()) {
        return;
      }
      buildMessage('clearCookie', key, null, callback);
    },
    wasInit: function () {
      return wasInit;
    }
